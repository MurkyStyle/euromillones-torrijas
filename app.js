...
function renderPlayers(){document.querySelector('#playersGrid').innerHTML=players.map(p=>`<article class="player-card"><div class="player-head"><span>${p.name}</span><span>${euro(totalFor(p.name))}</span></div>${balls(p.numbers,p.stars)}</article>`).join('')}
...
function render(){renderYearTabs();renderPlayers();renderSummary();renderStats();renderHistory();renderGlobal()}
...
document.querySelector('#notifyButton').addEventListener('click',async()=>{if(!('Notification'in window)){alert('Este navegador no permite notificaciones.');return}const p=await Notification.requestPermission();if(p==='granted')new Notification('Euromillones',{body:'Avisos activados para las actualizaciones de resultados.'})});
document.querySelector('#whatsappButton').addEventListener('click',()=>{const latest=[...activeDraws()].sort((a,b)=>b.date.localeCompare(a.date))[0];const text=latest?`🎱 Euromillones — Los Torrijas · ${latest.date}\nNúmeros: ${latest.numbers.join(', ')}\nEstrellas: ${latest.stars.join(', ')}\nPremios de Los Torrijas: ${euro(Object.values(latest.prizes).reduce((a,b)=>a+b,0))}`:'🎱 Euromillones — Los Torrijas ya tienen sus seis combinaciones cargadas.';window.open('https://wa.me/?text='+encodeURIComponent(text),'_blank')});
