...
function renderPlayers(){document.querySelector('#playersGrid').innerHTML=[...players].sort((a,b)=>totalFor(b.name)-totalFor(a.name)).map(p=>`<article class="player-card"><div class="player-head"><span class="player-name">${avatar(p)}${p.name}${p.paid?'<span class="payment-status" title="Ingreso realizado" aria-label="Ingreso realizado">€</span>':''}</span><span>${euro(totalFor(p.name))}</span></div>${balls(p.numbers,p.stars)}</article>`).join('')}
function totalFor(name){return activeDraws().reduce((sum,d)=>sum+(d.prizes[name]||0),0)}
function accountSnapshot(){const start=new Date(`${accountStartDate}T12:00:00`),today=new Date();today.setHours(12,0,0,0);let mondays=0;for(const date=new Date(start);date<=today;date.setDate(date.getDate()+1))if(date>start&&date.getDay()===1)mondays+=1;const topUps=accountTopUps.filter(topUp=>new Date(`${topUp.date}T12:00:00`)<=today).reduce((sum,topUp)=>sum+topUp.amount,0),prizes=draws.filter(draw=>draw.date>accountStartDate).reduce((sum,draw)=>sum+Object.values(draw.prizes||{}).reduce((total,amount)=>total+amount,0),0),balance=Math.round((accountStartBalance+topUps-accountSettledWeeklyCosts-(mondays*30)+prizes)*100)/100;return {balance,mondays,weeks:Math.max(0,Math.floor(balance/30)),low:balance<61}}
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>{let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!refreshing){refreshing=true;window.location.reload()}});navigator.serviceWorker.register('./sw.js?v=20260819-3',{updateViaCache:'none'}).then(registration=>{registration.update();setInterval(()=>registration.update(),60*60*1000)}).catch(()=>{})});
render();
