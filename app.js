...
async function loadLatestResults(){try{const response=await fetch('./data/results.json',{cache:'no-store'});if(!response.ok)return;const data=await response.json();if(!Array.isArray(data.draws))return;const remoteDraws=data.draws.map(drawFromRemote);const combined=new Map([...officialDraws,...remoteDraws,...savedDraws].map(draw=>[draw.date,draw]));draws=[...combined.values()];render();checkPrizeNotification()}catch{}}
